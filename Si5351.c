...
uint32_t P1_floor = (128 * ((float)num / (float)denom));
P1 = (uint32_t)(128 * (uint32_t)(mult) + P1_floor - 512);

uint32_t P2_floor = (128 * ((float)num / (float)denom));
P2 = (uint32_t)(128 * num - denom * P2_floor);
...
uint8_t clkCtrlReg = 0x0F;
  ...
  P3 = 1;*/
uint32_t P1_floor = (128 * ((float)num / (float)denom));
P1 = (uint32_t)(128 * (uint32_t)(div) + P1_floor - 512);
...
uint32_t _floor(uint32_t a, uint32_t b){
	uint32_t div = a/b; 
	return div;
}
...
void set_LO_freq(uint32_t freq)
{
    ...
    uint32_t fvco = 700000000, div, num, denom, rdiv;
uint32_t xtalfreq = 250000000;

div = floor(fvco/freq);
/*uint32_t scaling = _gcd(freq, xtalfreq);
uint32_t y2 = min(xtalfreq/scaling, 1048757);
uint32_t x2 = freq*div*y2/xtalfreq;
div =floor(_floor(x2, y2));*/
num = fvco%freq;
denom = freq;
...
/*
  ...
  freq = freq / 100 * 100;
  div = fvco / freq;
uint32_t pllfreq = div*freq;
  num = (pllfreq % freq)*1048575/freq;
  denom = freq;

      //rdiv = fvco / freq;
  //uint32_t remainder = fvco % freq;
...
divider = 700000000 / freq;// Calculate the division ratio. 900,000,000 is the maximum internal
// PLL frequency: 900MHz
if (divider % 2) divider--; // Ensure an even integer division ratio

pllFreq = divider * freq; // Calculate the pllFrequency: the divider * desired output frequency
...
if(8000000 <= freq && freq < 9000000){
mult = mult - 2;
mult = mult - 2;
} else if (9000000 <= freq && freq < 10000000){
mult = mult -3 ;
mult = mult -3;
}else if (10000000 <= freq && freq < 11000000){
mult = mult -4 ;
mult = mult -4;
}*/
...
set_phase(round(700000000/freq));
}